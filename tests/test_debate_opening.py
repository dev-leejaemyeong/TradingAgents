"""The first speaker in each debate must not rebut a nonexistent argument
(upstream #1176, ported 2026-09-08).

Each debate round's opening speaker receives an empty opponent response; the
prompt used to interpolate it into a "refute the opponent" instruction, so
models fabricated the other side's position. With this project's production
default (``max_debate_rounds``/``max_risk_discuss_rounds`` == 1), every
debator is always the opening speaker, so this is not a rare first-round edge
case here -- it fires on every live debate turn. All five debators (bull,
bear, and the three risk analysts) now substitute an explicit opening marker
when the opponent has not spoken, and pass a real argument through unchanged.
"""
...
def _capturing_llm(captured: dict):
    """A MagicMock LLM (not ChatAnthropic) so cached_blocks() returns a plain
    joined string; invoke() is still called with ``[HumanMessage(content=...)]``
    by every debator node, so the prompt text is recovered via ``.content``."""
    llm = MagicMock()
