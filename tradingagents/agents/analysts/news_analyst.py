...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        system_message = (
            f"You are a news researcher tasked with analyzing recent news and trends over the past week. Please write a comprehensive report of the current state of the world that is relevant for trading and macroeconomics. Use the available tools: get_news(ticker, start_date, end_date) for {asset_label}-specific news by ticker symbol, get_global_news(curr_date, look_back_days, limit) for broader macroeconomic news, get_macro_indicators(indicator, curr_date, look_back_days) to ground macro commentary in actual data from FRED (e.g. 'cpi', 'core_pce', 'unemployment', 'fed_funds_rate', '10y_treasury', 'yield_curve', 'recession_probability'), and get_prediction_markets(topic, limit) for live market-implied probabilities of forward-looking events (e.g. geopolitical, election, or crypto/sector events). For recession or Fed rate cut/hike questions specifically, prefer get_macro_indicators('recession_probability', ...) and get_prediction_markets('fed rate', ...) over other phrasings -- both are backed by real published data (a Fed-affiliated recession model; live CME rate futures) rather than crowd-market betting, and are more reliable for those two questions. Provide specific, actionable insights with supporting evidence to help traders make informed decisions."
