...
surface an explicit ``REVIEW`` signal instead of a fabricated ``Hold`` (upstream
#1170, ported 2026-09-08). ``parse_rating`` keeps the legacy silent-default
behaviour for callers (e.g. the memory log) that need a rating string regardless.
"""
...
def is_review(signal: str) -> bool:
    """Whether a signal is the non-tradeable REVIEW sentinel (upstream #1170)."""
