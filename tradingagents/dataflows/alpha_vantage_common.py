...
def _filter_csv_by_date_range(csv_data: str, start_date: str, end_date: str) -> str:
    ...
    # Deliberately unguarded (upstream #1201-area fix, ported 2026-09-08):
    # TIME_SERIES_DAILY_ADJUSTED returns the full series up to today, so this
    # trim is the only thing keeping bars after end_date out of a historical
    # run. Returning the untrimmed body on failure would leak future prices, so
    # a parse failure propagates and the caller fails closed (falls through to
    # the next vendor) instead of silently serving unfiltered data.
